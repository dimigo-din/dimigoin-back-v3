...
const detsSchema = createSchema({
  ...
  time: Type.array({ required: true, enum: TimeValues }), // add time type validation
  speakerID: Type.ref(Type.objectId()).to('User', userSchema),
  day: Type.array({ required: true, enum: DayValues }), // add day type validation
  room: Type.string({ required: true }), // add room type validation
  maxcount: Type.number({ required: true }),
  targetGrade: Type.number({ required: true, enum: GradeValues }), // add grade validation

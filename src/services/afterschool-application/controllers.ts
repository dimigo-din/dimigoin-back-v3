...
import * as AfterschoolApp from '../../models/afterschool-application';
...
export const applyAfterschool = async (req: Request, res: Response) => {
  ...
  if (await AfterschoolApp.checkOverlap(userId, afterschool._id)) {
    throw new HttpException(409, '중복 수강이 불가능한 강좌를 이미 신청했거나, 동시간대에 이미 신청한 강좌가 있습니다.');
  }

  const afterschoolApplication = new Afterschool.model({
    applier: userId,
...
export const cancelApplication = async (req: Request, res: Response) => {
  ...
  const afterschoolApplication = await Afterschool.model.findOne({
